...
**人机协同扩展 helper（本插件内置，已注入 globals，直接调用）**
\`\`\`python
ax_list(limit=100)                    # 可交互元素清单 [{role,name,cx,cy,backendNodeId}]
ax_click(role=None, text=None, index=0)   # 按角色/文本定位并点击，返回命中元素
form_fields()                         # 表单字段清单 [{tag,type,name,id,label,value,required}]
fill_field(name_or_label, value)      # 按字段名/标签自动填单个控件（兼容 React/Vue）
show_tab()                            # 把当前页切到 Chrome 前台（「轮到你」信号）
wait_url_change(old_url=None, timeout=120)   # 轮询等待页面跳转（如用户登录完成）
page_brief(max_text=600)              # 一屏概况 {url,title,elements,text}
\`\`\`
标准找点点击链路已封装：**优先 ax_list() / ax_click()，不要手写 getFullAXTree+getBoxModel**，
除非 ax_list 覆盖不了（如 canvas 内元素）。

**人机协同流程（表单申请类任务的标准打法）**
1. \`new_tab(url)\` + \`show_tab()\` —— 让用户看到页面
2. \`form_fields()\` 摸清表单 → **能自动填的**（姓名/邮箱/项目名等普通字段）逐个 \`fill_field()\`
3. 遇到**登录、验证码、扫码、支付、身份证件**等必须本人操作的步骤：
   \`show_tab()\` 后**停下来明确告诉用户**「这一步请在浏览器里完成 XXX，完成后告诉我」，
   然后 \`wait_url_change(旧url, timeout=180)\` 等用户做完
4. 用户确认后继续 ax_list/fill_field 推进下一步
5. 全部完成 → \`page_brief()\` 向用户汇报结果
...
/**
 * Autopilot（Jev）用法段 —— 仅在配置了 TypeSafe key 后才随 HARNESS_API 一起注入。
 * 没配置 key 时模型看不到这段，也就不会去调 browser_autopilot（条件启用的提示词侧）。
 */
export const AUTOPILOT_API = `
### 目标式自动化（browser_autopilot —— 已启用）

你额外有一个 **目标式** 工具：把「终点」交给 Jev（TypeSafe 的 System One 浏览器模型，
非 LLM、读页面 DOM 做毫秒级结构化决策），它自主完成整个操作循环。

\`\`\`
browser_autopilot({ url, goal, lane?, timeoutMs? })
\`\`\`

- \`url\` 起始页面；\`goal\` 写**终点**（如「完成开发者账号注册并进入控制台」），**不要写步骤** —— 怎么点、点什么由它自己决策
- 与 \`browser_run\` 共享同一个浏览器；传相同 \`lane\` 就在同一条车道（同一标签页）上跑
- 适合「把某网站的某流程走完」这类粗粒度目标；需要精确控制每一步时仍用 \`browser_run\`
- 返回 \`{ok, exitCode, stdout, stderr}\`；遇到登录/验证码/支付它会停下交还用户，之后可用 \`browser_run\` 接手继续
`.trim();
