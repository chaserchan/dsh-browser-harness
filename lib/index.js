/**
 * dsh-browser-harness —— 浏览器控制插件（host-only）
 *
...
import { AUTOPILOT_API, HARNESS_API } from './harness-api.js'

/**
 * 人机协同 helper 集（ax_list / ax_click / form_fields / fill_field /
 * show_tab / wait_url_change / page_brief）。每个 browser_run 执行前注入到
 * globals，用户代码零 import 直调 —— 消灭最易写错的手搓 CDP 链路。
 */
const HELPERS_SRC = readFileSync(new URL('./harness-helpers.py', import.meta.url), 'utf8')
...
function resolveConfig(config = {}) {
  ...
  return {
    ...
    buPrefix: config.buPrefix ?? process.env.DSH_BROWSER_USE_BU_PREFIX ?? 'dsh',
    ...
    autopilotCommand: config.autopilotCommand ?? '',
  }
}
 ...
 */
function runHarness(cfg, code, cdpUrl, timeoutMs, buName) {
  const { cmd } = resolveCommand(cfg.command)
  const env = harnessEnv(cfg, cdpUrl, buName)
  ...
  return new Promise((resolve) => {
    ...
    // HELPERS_SRC：人机协同 helper 集（ax_list/ax_click/form_fields/fill_field/
    // show_tab/wait_url_change/page_brief），定义在同一 globals，用户代码直调。
...
/**
 * 解析 autopilot（jev-ultrafast）运行体命令。
 * jev 的实际安装形态未经本机实测，所以执行体是配置驱动而非写死：
 * 配置了 autopilotCommand 就用，没配置就明确告知怎么装，绝不瞎猜命令。
 */
function resolveAutopilotCommand(cfg) {
  if (cfg.autopilotCommand) return { cmd: cfg.autopilotCommand, source: 'config' }
  return { cmd: '', source: 'none' }
}

/**
 * 跑一次 autopilot：spawn 配置的 jev 命令，追加 --url/--goal，
 * 并透传 TypeSafe/TextModel key 与 lane 对应的 daemon 环境变量，
 * 让 Jev 附着与本插件同一个浏览器（官方并发形态）。
 */
function runAutopilot(cfg, keys, url, goal, timeoutMs, buName, cdpUrl) {
  const { cmd, source } = resolveAutopilotCommand(cfg)
  if (!cmd) {
    return Promise.resolve({
      ok: false, exitCode: null, stdout: '', stderr: 'autopilot command not configured',
      hint: 'TypeSafe key 已配置，但还没有可执行的 jev 运行体。先安装 jev-ultrafast，'
        + '再在「设置 → 通用设置 → 浏览器自动化」所在插件的 cordis.patch.yml 里配 autopilotCommand（例如 "uvx jev-ultrafast"）。',
    })
  }
  const argv = cmd.split(/\s+/).filter(Boolean)
  const env = {
    ...harnessEnv(cfg, cdpUrl, buName),
    TYPESAFE_API_KEY: keys.typesafe,
    ...(keys.textModel ? { TEXT_MODEL_API_KEY: keys.textModel } : {}),
  }
  return new Promise((resolve) => {
    let child
    try {
      child = spawn(argv[0], [...argv.slice(1), '--url', url, '--goal', goal], { env, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] })
    } catch (e) {
      resolve({ ok: false, exitCode: null, stdout: '', stderr: String(e?.message ?? e), hint: `autopilot 命令启动失败（source=${source}）` })
      return
    }
    let stdout = ''
    let stderr = ''
    ...
    const timer = setTimeout(() => {
      ...
      settled = true
      try { child.kill('SIGKILL') } catch { /* 进程可能已退出 */ }
      resolve({ ok: false, exitCode: null, stdout, stderr, hint: `autopilot 执行超时（${timeoutMs}ms）。目标较大时用 timeoutMs 提高上限（最大 ${MAX_TIMEOUT_MS}ms）。` })
    }, timeoutMs)
    child.on('error', (err) => {
      if (settled) return
      settled = true
      clearTimeout(timer)
      resolve({ ok: false, exitCode: null, stdout, stderr: String(err?.message ?? err), hint: `autopilot 命令执行失败（source=${source}）。先 browser_status 看解析情况。` })
    })
    child.on('close', (code) => {
      if (settled) return
      settled = true
      clearTimeout(timer)
      resolve({ ok: code === 0, exitCode: code, stdout: stdout.trimEnd(), stderr: stderr.trimEnd() })
    })
  })
}
 ...
 * 锁按 lane 分把 —— 同车道（同 BU_NAME/同 tab）串行；跨车道真并行。
 */
const laneLocks = new Map()
const activeLanes = new Map() // lane -> { lastUsed, calls }
function serialize(lane, fn) {
  let chain = laneLocks.get(lane)
  if (!chain) { chain = Promise.resolve(); laneLocks.set(lane, chain) }
  const next = chain.then(fn, fn)
  chain = next.then(() => undefined, () => undefined)
  return next
}

/** 从工具执行上下文里尽力取一个稳定的会话键（取不到返回 null）。 */
function probeSessionKey(exec) {
  try {
    const a = exec && exec.agent
    if (!a) return null
    const candidates = [a.sessionId, a.session && a.session.id, a.id, a.name]
...
export function apply(ctx, config = {}) {
  ...
  // 设置面板写入的 key 存在这个 namespace（browser-harness.typesafeKey 等）。
  // settings provider 热加载，保存即生效，无需重启 —— 与 global-prompt 同一机制。
  let harnessSettings = null
  /** 实时读 key：设置面板 > 环境变量 > 空。每次调用都重读，保证保存后立即生效。 */
  const readKeys = () => {
    let stored = {}
    try { stored = harnessSettings?.get() ?? {} } catch { /* settings 未就绪时不阻塞 */ }
    return {
      typesafe: String(stored.typesafeKey ?? '').trim() || String(process.env.TYPESAFE_API_KEY ?? '').trim(),
      textModel: String(stored.textModelKey ?? '').trim() || String(process.env.TEXT_MODEL_API_KEY ?? '').trim(),
    }
  }

  const runBody = async (code, timeoutMs, buName) => {
    ...
    return runHarness(cfg, code, cdpUrl, timeoutMs, buName)
  }

  /** 解析一次调用的 lane（浏览器隔离车道）：显式 lane > exec 会话键 > default。 */
  function laneOf(args, exec) {
    if (args.lane && typeof args.lane === 'string' && args.lane.trim()) return args.lane.trim()
    return probeSessionKey(exec) || 'default'
  }
  ...
  ctx.inject(['tools'], (sctx) => {
    sctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        lane: { type: 'string', description: '浏览器隔离车道名（如任务代号）。同一 lane 复用同一标签页且串行执行；不同 lane 各自独立标签页可并行。多任务并行时给每个任务固定一个 lane；单任务不用传。' },
      },
      output: { schema: { type: 'json' }, render: renderJson },
      execute: (args, exec) => {
        const lane = laneOf(args, exec)
        activeLanes.set(lane, { lastUsed: Date.now(), calls: (activeLanes.get(lane)?.calls ?? 0) + 1 })
        const buName = `${cfg.buPrefix}-${lane}`
        return serialize(lane, () => runBody(args.code, clampTimeout(args.timeoutMs ?? cfg.timeoutMs), buName))
      },
    }))
    ...
    sctx.tools.register(defineTool({
      ...
      execute: () => serialize(async () => {
        ...
        return {
          ...
          lanes: {
            perSession: true,
            active: [...activeLanes.entries()].map(([lane, v]) => ({
              lane,
              calls: v.calls,
              lastUsedAgoSec: Math.round((Date.now() - v.lastUsed) / 1000),
              daemon: `${cfg.buPrefix}-${lane}`,
            })),
            note: 'browser_run 传 lane 参数即独占一条浏览器车道（独立标签页、互不抢占）；不传自动按会话隔离。',
          },
          cdpUrl: cdpUrl || '(附着系统浏览器，由 Harness 自行发现)',
          userDataDir: cfg.mode === 'dedicated' ? cfg.userDataDir : '(不适用)',
          autopilot: (() => {
            const keys = readKeys()
            const ap = resolveAutopilotCommand(cfg)
            return {
              typesafeKey: keys.typesafe ? '(已配置)' : '(未配置)',
              textModelKey: keys.textModel ? '(已配置)' : '(未配置)',
              command: ap.cmd || '(未配置)',
              commandSource: ap.source,
              note: keys.typesafe
                ? 'browser_autopilot 可用' + (ap.cmd ? '' : '（但 jev 运行体未配置，见 README「Autopilot」）')
                : '未配置 TypeSafe key：设置 → 通用设置 → 浏览器自动化，填入保存后立即可用',
            }
          })(),
    ...
    // Autopilot（Jev）：条件启用 —— 用户语义「没配置 key 就不使用，配置了就使用」。
    // 工具本体始终注册（工具表运行时增删不是稳定契约），但没 key 时执行直接拒绝并
    // 引导去设置页；同时系统提示词不注入 autopilot 段，模型根本不知道有这个工具。
    sctx.tools.register(defineTool({
      name: 'browser_autopilot',
      description:
        '目标式浏览器自动化（需在设置里配置 TypeSafe key 才可用）：把一个「终点目标」交给 Jev，'
        + '它读页面 DOM 自主完成决策循环，适合「把 X 网站的某流程走完」这类任务。'
        + '与 browser_run 共享浏览器和 lane 车道。goal 写清楚终点而不是步骤。',
      parameters: {
        url: { type: 'string', required: true, description: '起始页面 URL。' },
        goal: { type: 'string', required: true, description: '终点目标，如「完成开发者账号注册并进入控制台」。写终点，不要写操作步骤。' },
        lane: { type: 'string', description: '与 browser_run 同语义：浏览器隔离车道名，同一 lane 复用同一标签页。' },
        timeoutMs: { type: 'integer', description: `执行预算（毫秒），默认 ${DEFAULT_TIMEOUT_MS}，上限 ${MAX_TIMEOUT_MS}。` },
      },
      output: { schema: { type: 'json' }, render: renderJson },
      execute: (args, exec) => {
        const keys = readKeys()
        if (!keys.typesafe) {
          return Promise.resolve({
            ok: false, exitCode: null, stdout: '', stderr: 'typesafe key not configured',
            hint: '未配置 TypeSafe key，autopilot 不可用。打开 设置 → 通用设置 → 浏览器自动化，'
              + '填入 TypeSafe API Key 保存后立即可用（无需重启）。',
          })
        }
        const lane = laneOf(args, exec)
        activeLanes.set(lane, { lastUsed: Date.now(), calls: (activeLanes.get(lane)?.calls ?? 0) + 1 })
        const buName = `${cfg.buPrefix}-${lane}`
        return serialize(lane, () => {
          const timeout = clampTimeout(args.timeoutMs ?? cfg.timeoutMs)
          if (cfg.mode === 'dedicated') {
            const ready = ensureDedicatedChrome(cfg)
            if (!ready.ok) return { ok: false, exitCode: null, stdout: '', stderr: ready.error, hint: ready.hint }
          }
          return runAutopilot(cfg, keys, String(args.url), String(args.goal), timeout, buName, cdpUrl)
        })
      },
    }))

    console.log(`[dsh-browser-harness] 已注册 3 个浏览器工具（mode=${cfg.mode}, command=${resolvedCmd}）`)
  })

  // 设置面板需要的持久字段：TypeSafe key（Jev autopilot）+ Text Model key（可选，
  // 供 Jev 的兜底文本模型用，OpenAI 兼容端点均可）。host 侧只注册 schema 与存取。
  ctx.inject(['settings'], (sctx) => {
    harnessSettings = sctx.settings.register('browser-harness', z.object({
      typesafeKey: z.string().default('').max(200),
      textModelKey: z.string().default('').max(200),
    }))
  })

  // 把 Harness 的可用函数表与操作纪律注入系统提示词。
  // 只给模型一个 Python 执行口、不告诉它有什么函数，模型必然瞎写 —— 这段是成败关键。
  // text 是函数、每步重估：TypeSafe key 配置了才追加 autopilot 段（「没配置就不使用」
  // 的另一半 —— 没配置时模型连这个工具的存在都看不到）。
  ctx.inject(['systemPrompt'], (sctx) => {
    sctx.systemPrompt.section({
      name: 'user:browser-harness',
      order: 60,
      text: () => (readKeys().typesafe ? HARNESS_API + '\n\n' + AUTOPILOT_API : HARNESS_API),
    })
  })
}
...
// closure pruned 20260919: client-half packages removed (dupe slot registration fix)
