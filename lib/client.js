...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function apply(ctx) {
			// 0.1.7+：settingsScope 不存在时降级 —— 不注册设置行、不 pending，
			// 等 ConfigForms 新架构适配后恢复（task: settings API 适配）。
			if (!ctx.settingsScope) {
				if (typeof console !== "undefined") {
					console.warn("[dsh-browser-harness] client: settingsScope 服务不存在（0.1.7 重构），设置行未注册；TypeSafe key 请用环境变量 TYPESAFE_API_KEY 配置（立即生效）。");
				}
				return;
			}
