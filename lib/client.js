...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 本插件注入 configForms，用它做后端重新提供 settingsScope（兼容垫片，见 apply）。
		const inject = ["slots", "locale", "configForms"];

		/**
		 * 旧 settingsScope 契约 → 0.1.7 configForms 适配。
		 *
		 * 旧契约（消费方实测）：bind({ namespace }) → { subscribe, getSnapshot, set }
		 *   - getSnapshot() → { status: "ready", value: {…字段} }（消费方检查 status）
		 *   - subscribe(fn) → disposer（fn 可能带 snapshot 参数，也可能不带）
		 *   - set(field, value) → Promise
		 * configForms.get(namespace) 提供 subscribe / getSnapshot().value / set 同构面，
		 * 此处补齐 status 字段并隔离消费方异常。
		 */
		function makeCompatScope(ctx, namespace) {
			let form = null;
			try {
				form = ctx.configForms.get(namespace);
			} catch (e) {
				form = null; // namespace 未在 host 注册（第三方 host 半边在 0.1.7 下未能注册）
			}
			const snap = () => {
				try {
					const s = form.getSnapshot();
					return { status: "ready", value: (s && typeof s === "object" && s.value) ? s.value : {} };
				} catch (e) {
					return { status: "unavailable", value: {} };
				}
			};
			if (form && typeof form.subscribe === "function" && typeof form.set === "function") {
				return {
					subscribe: (fn) => form.subscribe(() => {
						try { fn(snap()); } catch (e) { /* 消费方异常隔离，不打断 fan-out */ }
					}),
					getSnapshot: snap,
					set: (field, value) => form.set(field, value),
				};
			}
			// 内存降级：可读可订阅（永不触发），写为 no-op —— 比 pending 不可用强
			return {
				subscribe: () => () => {},
		 ...
		 * Client plugin body.
		 *
		 * 0.1.7 兼容垫片（寄居于此，待抽为独立插件）：把旧版 settingsScope 服务以
		 * configForms 为后端重新提供。cordis 的 inject 是「等服务」语义 —— 服务一经
		 * 提供，所有仍在等待它的第三方 client 插件（session-cost / agent-message 等）
		 * 立即自动激活，无需改动它们一行代码。
		 *
		...
		function apply(ctx) {
			try {
				ctx.provide("settingsScope", { bind: ({ namespace }) => makeCompatScope(ctx, namespace) });
				if (typeof console !== "undefined") {
					console.info("[dsh-browser-harness] client: settingsScope 兼容垫片已启用（configForms 后端）");
				}
			} catch (e) {
				// cordis 对同名服务重复注册会抛错 —— 说明已有人提供，静默让位
				if (typeof console !== "undefined") {
					console.warn("[dsh-browser-harness] client: settingsScope 垫片未注册：", e && e.message ? e.message : e);
				}
			}
			// 本插件自身的设置行待 ConfigForms 原生适配（task #35）；
			// TypeSafe key 过渡期走环境变量 TYPESAFE_API_KEY / TEXT_MODEL_API_KEY（host 实时读取）。
		}
